...
interface HeroUiProps {
  profile: Profile | null; // Allow null for fallback
}

export function HeroUi({ profile }: HeroUiProps) {
  // Fallback data in case DB is empty or fresh
  ...
  // Note: Your Profile schema doesn't have an image field yet.
  // I'm using your hardcoded one, but you should add 'avatarUrl' to your Prisma model.
  const imageUrl =
    profile?.imageUrl ||
    "https://lh3.googleusercontent.com/aida-public/AB6AXuCBaQGw1e5sPQqEGKdgPAUOJhZnCZb2AOnBUh7yHcGDbAH-QZYCnIPjljyvVDhgPy55sW5_KPyNmArlZSB00-3KsLt3w3BvOifdBgyfUjrDK7YNWV0zKmEc0Jjjkd06S7k1dcchu9hEPrvvFNMEJeXw6kS43JGafck6xesXghzZMGiADKVrzxJuQUJz3Ci6_ugh5Jt32OLzPldStDMcetvMP8R7dh8GmkUrnOUso9WzXjLC79I_0UjWQq260ZcDLS9uKKiQYEY_RBZe";
    ...
    <section className="relative flex flex-col md:flex-row items-center justify-center min-h-[calc(100vh-80px)] w-full max-w-[1200px] mx-auto gap-8 lg:gap-16 px-6 md:px-10 pt-24 md:pt-0">
      ...
      >
        ...
        <Image
          ...
          alt={fullName}
          fill
          className="object-cover object-center transition-transform duration-700 group-hover:scale-105 filter hover:grayscale-0"
          priority
